...
const nav = [["Dashboard","/workspace"],["Invoices","/invoices"],["New invoice","/invoices/new"],["Audit log","/audit"],["Developers","/developers"]] as const;
export function AppShell({children}:{children:React.ReactNode}){
 return <div className="app-grid"><aside className="sidebar">
  <Link className="brand" href="/workspace"><span className="brand-mark">WE</span><span>Wedge Ethiopia</span></Link>
  <div className="nav-section">Operations</div><nav className="nav">{nav.map(([l,h])=><Link key={h} href={h}>{l}</Link>)}</nav>
  <div className="nav-section">Public</div><nav className="nav" style={{marginTop:0}}><Link href="/">Marketing site</Link></nav>
  ...
  <Link className="brand mobile-brand" href="/workspace"><span className="brand-mark">WE</span><span>Wedge Ethiopia</span></Link>
